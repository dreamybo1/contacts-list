...
const contacts = JSON.parse(storeContacts || "") || {};
...
letters.forEach(letter => {
    ...
    if (!Object.hasOwn(contacts, letter))
        contacts[letter] = [];
    if (list) {
...
content.onclick = (event) => {
    ...
    const letterEl = target.closest(".letter");
    //проверяем, что было нажатие на кнопку (делегируем)
    if (!letterEl)
        return;
    let eventDataEl = letterEl?.querySelector(".first-letter");
    let eventData = eventDataEl?.innerHTML;
    let infoEl = document.querySelector(".list-item__info");
